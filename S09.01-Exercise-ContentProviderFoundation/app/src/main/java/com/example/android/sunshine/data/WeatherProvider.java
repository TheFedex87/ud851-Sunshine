...
//      TODO (9) Handle queries on both the weather and weather with date URI
        ...
        switch (sUriMatcher.match(uri)){
            case CODE_WEATHER:
                returnCursor = db.query(WeatherContract.WeatherEntry.TABLE_NAME, projection, selection, selectionArgs, null, null, sortOrder);
                break;

            case CODE_WEATHER_WITH_DATE:
                String date = uri.getLastPathSegment();
                String[] arguments = new String[]{date};

                returnCursor = db.query(WeatherContract.WeatherEntry.TABLE_NAME, projection, WeatherContract.WeatherEntry.COLUMN_DATE + "=?", arguments, null, null, sortOrder);
                break;
